...
const arrayToObject = function (array, keys, gaurdiansFormat) {
  ...
  for (let index = 0; index < keys.length; index++) {
    object[keys[index]] = Array.isArray(array[index])
      ? getGuardianDetails(gaurdiansFormat)(array[index])
      : array[index];
  }
  return object;
};
...
const personYoungerThen20 = peopleRefinedDetails.filter(function (
  personDetail
) {
...
const studentParentNameAndPhoneNumber = detailsOfYoungerThen30.map(function (
  student
) {
  return [
    student.name,
    student.phoneNumber,
    student.guardinan.name,
    student.guardinan.phoneNumber,
  ];
});
...
const peopleOlderThen20WithValidParent = peopleOlderThen20.filter(function (
  people
) {
...
const validParentDetails = peopleOlderThen20WithValidParent.map(function (
  people
) {
